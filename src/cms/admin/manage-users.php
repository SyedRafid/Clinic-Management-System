...
if (strlen($_SESSION['id']) == 0) {
    ...
    if (isset($_GET['del'])) {
        ...
        $checkUserType = mysqli_query($con, "SELECT userType FROM users WHERE id ='$uid'");
        $userTypeRow = mysqli_fetch_assoc($checkUserType);
        $userType = $userTypeRow['userType'];

        if ($userType !== 'Admin') {
            $deleteDoctorsSql = mysqli_query($con, "DELETE FROM doctors WHERE id ='$uid'");
            $deleteUsersSql = mysqli_query($con, "DELETE FROM users WHERE id ='$uid'");

            if ($deleteDoctorsSql && $deleteUsersSql) {
                $_SESSION['msg'] = "Data deleted successfully!";
            } else {
                $_SESSION['error'] = "Error deleting data: " . mysqli_error($con);
            }
        } else {
            $_SESSION['msg'] = "You cannot delete an admin user!";
        }
    }
?>
<!DOCTYPE html>
