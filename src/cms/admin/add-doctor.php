...
if (strlen($_SESSION['id']) == 0) {
	...
	if (isset($_POST['submit'])) {
		...
		$users_sql = mysqli_query($con, "INSERT INTO users(fullName, address, city, gender, email, password, userType) VALUES ('$docname', '$docaddress', '$doccity', '$docgender', '$docemail', '$password', '$usertype')");

		if ($users_sql) {
			// Step 2: Retrieve the inserted user's ID
			$users_id = mysqli_insert_id($con);

			// Step 3: Insert data into the "doctors" table with the user's ID
			$doctors_sql = mysqli_query($con, "INSERT INTO doctors(id, specilization, docFees, contactno) VALUES ('$users_id', '$docspecialization', '$docfees', '$doccontactno')");

			if ($doctors_sql) {
				echo "<script>alert('Doctor info added Successfully');</script>";
				echo "<script>window.location.href ='manage-doctors.php'</script>";
			} else {
				echo "Error inserting data into doctors table: " . mysqli_error($con);
				echo "<script>window.location.href ='manage-doctors.php'</script>";
			}
		} else {
												...
												<div class="panel-body">
													...
													<form role="form" name="adddoc" method="post" onSubmit="return valid();">
														...
														<div class="form-group">
															...
															<input type="text" name="doccity" class="form-control" placeholder="Enter Doctor's City" required="true">
														</div>
														<div class="form-group">
